-- Load the input data
inputDialogues4 = LOAD 'hdfs:///user/Ragini/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/Ragini/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/Ragini/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/Ragini/outputs;

-- Store result in hdfs
STORE namesOrdered INTO 'hdfs:///user/Ragini/outputs' USING PigStorage('\t');
